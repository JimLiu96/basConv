...
class NGCF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        self.b_g_embeddings = tf.nn.embedding_lookup(self.ba_embeddings, self.baskets)
        ...
        self.b_at_embeddings = self.b_g_embeddings + self.u_c_embeddings
    ...
    def _init_weights(self):
        ...
        if self.pretrain_data is None:
            ...
            all_weights['basket_embedding'] = tf.Variable(tf.zeros([self.n_baskets, self.emb_dim], tf.float32), name='basket_embedding', trainable=False)
    ...
    def _create_ngcf_embed_ubi_R(self):
        ...
        for k in range(0, self.n_layers):
            ...
            temp_basket_embedding = tf.nn.leaky_relu(tf.matmul(basket_embedding[k], self.weights['W_self_%d' %k]) + tf.matmul(tf.multiply(u2b_embedding, 
                                basket_embedding[k]), self.weights['W_ub_%d' %k]) + tf.matmul(tf.multiply(i2b_embedding, basket_embedding[k]), 
                                self.weights['W_bi_%d' %k]))
            ...
            temp_user_embedding = tf.nn.leaky_relu(tf.matmul(user_embedding[k], self.weights['W_self_%d' %k]) + tf.matmul(tf.multiply(b2u_embedding, 
                                user_embedding[k]), self.weights['W_ub_%d' %k]) + tf.matmul(tf.multiply(i2u_embedding, user_embedding[k]), 
                                self.weights['W_ui_%d' %k]))
            ...
            temp_item_embedding = tf.nn.leaky_relu(tf.matmul(item_embedding[k], self.weights['W_self_%d' %k]) + tf.matmul(tf.multiply(b2i_embedding, 
                                item_embedding[k]), self.weights['W_bi_%d' %k]) + tf.matmul(tf.multiply(u2i_embedding, item_embedding[k]), 
                                self.weights['W_ui_%d' %k]))
            temp_basket_e_norm = tf.math.l2_normalize(temp_basket_embedding)
            temp_basket_e_norm = tf.nn.dropout(temp_basket_e_norm, 1 - self.mess_dropout[k])
...
if __name__ == '__main__':
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch==0 or (epoch > 50 and epoch%5==0) or epoch > 200):
